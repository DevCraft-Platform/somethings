...
class _utils_:
    ...
    def showChatDump(self, username: str, contact: str):
        database = self.client['webchat-app']
        users_collection = database['users']
        user = users_collection.find_one({"username": username})
        for index, contact in enumerate(user['contacts']):
            if 'username' in contact:
                print(f"Chat with: {contact['username']}")
                print(f"Chat ID: {contact['chatid']}")
                # Show the messages in a cool way
                for message in contact['messages']:
                    print(f"Message: {message['message']}")
                    print(f"Sender: {message['sender']}")
                    print(f"Receiver: {message['receiver']}")
                    print(f"Created at: {message['created_at']}")
                    print(f"{colorama.Fore.RED}-{colorama.Fore.RESET}" * 20)
                return True
        return False
class core:
    def __init__(self) -> None:
    ...
    def execute_list_chats(self):
        os.system("clear")
        print("Listing all chats!")
        print("-" * 20)
        time.sleep(2)
        username = self.utils.getUsers()
        username_search = survey.routines.select("Select the user to list the chats: ", options=username)
        contacts = self.utils.retriveContactsFromUser(username=username[username_search])
        if not contacts:
            print("You do not have any contacts to list!")
            sys.exit(1)
        else:
            contacts_list = [contact['username'] for contact in contacts]
            print(f"Listing all chats for {username[username_search]}")
            for contact in contacts_list:
                print(f"Chat with: {contact}")
        time.sleep(2)
        
    def execute_search_chat(self):
        os.system("clear")
        print("Search for a chat!")
        print("-" * 20)
        time.sleep(2)
        username = self.utils.getUsers()
        username_search = survey.routines.select("Select the user to search the chat: ", options=username)
        contacts = self.utils.retriveContactsFromUser(username=username[username_search])
        if not contacts:
            print("You do not have any contacts to search!")
            sys.exit(1)
        else:
            contacts_list = [contact['username'] for contact in contacts]
            contact_search = survey.routines.select("Select the contact to search the chat: ", options=contacts_list)
            print(f"Searching for chat with: {contacts_list[contact_search]}")
        time.sleep(2)
        
    def execute_remove_chat(self):
        os.system("clear")
        print("Remove a chat!")
        print("-" * 20)
        time.sleep(2)
        username = self.utils.getUsers()
        username_search = survey.routines.select("Select the user to remove the chat: ", options=username)
        contacts = self.utils.retriveContactsFromUser(username=username[username_search])
        if not contacts:
            print("You do not have any contacts to remove!")
            sys.exit(1)
        else:
            contacts_list = [contact['username'] for contact in contacts]
            contact_search = survey.routines.select("Select the contact to remove the chat: ", options=contacts_list)
            print(f"Removing chat with: {contacts_list[contact_search]}")
        time.sleep(2)
        
    def execute_dump_chat(self):
        os.system("clear")
        print("Dump a chat!")
        print("-" * 20)
        time.sleep(2)
        username = self.utils.getUsers()
        username_search = survey.routines.select("Select the user to dump the chat: ", options=username)
        contacts = self.utils.retriveContactsFromUser(username=username[username_search])
        if not contacts:
            print("You do not have any contacts to dump!")
            sys.exit(1)
        else:
            contacts_list = [contact['username'] for contact in contacts]
            contact_search = survey.routines.select("Select the contact to dump the chat: ", options=contacts_list)
            print(f"Dumping chat with: {contacts_list[contact_search]}")
            # Now we need to dump the chat
            # connect to the database
            _utils_().showChatDump(username=username[username_search], contact=contacts_list[contact_search])
        time.sleep(2)